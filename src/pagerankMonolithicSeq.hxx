...
template <class T>
int pagerankMonolithicSeqLoopU(vector<T>& a, vector<T>& r, vector<T>& c, const vector<T>& f, const vector<int>& vfrom, const vector<int>& efrom, int i, int n, int N, T p, T E, int L, int EF) {
  int l = 0;
  while (l<L) {
    T c0 = pagerankTeleport(r, f, N, p);
    ...
    multiplyValuesPositiveW(c, a, f, i, n);            // update partial contributions (c)
...
PagerankResult<T> pagerankMonolithicSeqPair(const G& x, const H& xt, const vector<T> *q=nullptr, const PagerankOptions<T>& o={}) {
...
PagerankResult<T> pagerankMonolithicSeq(const G& x, const vector<T> *q=nullptr, const PagerankOptions<T>& o={}) {
  ...
  return pagerankMonolithicSeqPair(x, xt, q, o);
}
...
template <class G, class H, class T=float>
PagerankResult<T> pagerankMonolithicSeqDynamicPair(const G& x, const H& xt, const G& y, const H& yt, const vector<T> *q=nullptr, const PagerankOptions<T>& o={}) {
...
PagerankResult<T> pagerankMonolithicSeqDynamic(const G& x, const G& y, const vector<T> *q=nullptr, const PagerankOptions<T>& o={}) {
  ...
  return pagerankMonolithicSeqDynamicPair(x, xt, y, yt, q, o);
}
