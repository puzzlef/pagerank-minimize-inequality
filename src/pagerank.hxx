...
struct PagerankOptions {
  ...
  PagerankOptions(vector<T> *factors=nullptr, int repeat=1, int toleranceNorm=1, T damping=0.85, T tolerance=1e-6, int maxIterations=500) :
  factors(factors), repeat(repeat), toleranceNorm(toleranceNorm), damping(damping), tolerance(tolerance), maxIterations(maxIterations) {}
};
